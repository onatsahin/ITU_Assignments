...
void File:: updatef(){
	Phone_Record temprec;
	FILE *temptxt = fopen("temp.txt","w+");
	fseek(phonebook,0,SEEK_SET);
	while(!feof(phonebook)){
		fread(&temprec,sizeof(Phone_Record),1,phonebook);
		if(temprec.name == "" || temprec.phonenum == "") continue;
		else{
			fwrite(&temprec,sizeof(Phone_Record),1,temptxt);
		}
	}
	fclose(phonebook);
	fseek(temptxt,0,SEEK_SET);
	FILE *ptr2 = fopen("phonebook.txt","w");
	while(!feof(temptxt)){
		fread(&temprec,sizeof(Phone_Record),1,temptxt);
		fwrite(&temprec,sizeof(Phone_Record),1,phonebook);
	}
